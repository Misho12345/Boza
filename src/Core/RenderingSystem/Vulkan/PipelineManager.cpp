...
namespace boza
{
    ...
    void PipelineManager::bind_pipeline(const VkCommandBuffer command_buffer, const pipeline_id id)
    {
        vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, get_pipeline(id).get_pipeline());
    }

    Pipeline& PipelineManager::get_pipeline(const pipeline_id id)
    {
        auto& inst = instance();
    ...
    void PipelineManager::destroy_pipeline(const pipeline_id id)
    {
        auto& inst = instance();
        assert(inst.pipelines.contains(id) && "Pipeline not found");
