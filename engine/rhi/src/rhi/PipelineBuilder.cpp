...
namespace boza::rhi
{
    ...
    {
        std::unordered_map<uint32_t, std::vector<DescriptorBinding>> bindings_by_set;
    ...
    bool PipelineBuilder::build_descriptor_set_layouts()
    {
        ...
        for (uint32_t set_idx = 0; set_idx <= max_set; ++set_idx)
        {
            ...
            if (!layout)
            {
                Logger::error("Failed to create descriptor set layout for set {}", set_idx);
                return false;
            }
        ...
        const PrimitiveTopology      topology) const
    {
        if (!pipeline_layout_)
        {
            Logger::error("Pipeline layout not created. Call build_pipeline_layout() first.");
            return nullptr;
        }
        ...
        Buffer*            buffer) const
    {
        ...
        {
            Logger::error("Vertex attribute '{}' not found in shader", attribute_name);
            return false;
        }
        ...
        uint32_t           range) const
    {
        ...
        {
            Logger::error("Uniform buffer '{}' not found in shader", name);
            return false;
        }
        ...
        {
            Logger::error("Resource '{}' is not a uniform buffer", name);
            return false;
        }

        if (info.set >= descriptor_sets_.size())
        {
            Logger::error("Descriptor set {} not allocated for resource '{}'", info.set, name);
            return false;
        }
        ...
        uint32_t           range)
    {
        if (!resource_map_.contains(name))
        {
            Logger::error("Storage buffer '{}' not found in shader", name);
            return false;
        }
        ...
        {
            Logger::error("Resource '{}' is not a storage buffer", name);
            return false;
        }
    ...
    bool PipelineResourceBinder::update_sampler(const std::string& name, Texture* texture, Sampler* sampler)
    {
        if (!resource_map_.contains(name))
        {
            Logger::error("Sampler '{}' not found in shader", name);
            return false;
        }
        ...
        {
            Logger::error("Resource '{}' is not a combined image sampler", name);
            return false;
        }
