...
export namespace boza
{
    ...
    {
    public:
        static ComputeDispatcher& create(const std::string& shader_name, bool& failed);
        static void cleanup();

        template<typename T>
        ComputeDispatcher& set(const std::string& name, const T& value);

        ComputeDispatcher& set(const std::string& name, Texture* texture);
        ComputeDispatcher& set(const std::string& name, Buffer* buffer);
