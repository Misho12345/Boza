...
namespace boza::app
{
    ...
    bool GameSettings::load_from_file(const fs::path& filepath)
    {
        ...
        if (data.contains("gameplay") && data["gameplay"].is_object())
        {
            ...
            if (g.contains("target_fps") && g["target_fps"].is_number()) gameplay.target_fps = g["target_fps"].get<float>();
            if (g.contains("vsync") && g["vsync"].is_boolean()) gameplay.vsync = g["vsync"].get<bool>();
            if (g.contains("physics_update_rate") && g["physics_update_rate"].is_number()) gameplay.physics_update_rate = g["physics_update_rate"].get<float>();
        }
