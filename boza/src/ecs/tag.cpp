...
namespace boza
{
    Tag::Tag(const std::string& name) : id_{ ecs::TagManager::instance().tag_id(name) } {}
    ...
    {
        id_ = ecs::TagManager::instance().tag_id(tag_name);
        return *this;
    }
    ...
    bool Tag::operator!=(const Tag& other) const { return id_ != other.id_; }
    bool Tag::operator==(const std::string& tag_name) const { return id_ == ecs::TagManager::instance().tag_id(tag_name); }
    bool Tag::operator!=(const std::string& tag_name) const { return !(*this == tag_name); }

    std::uint32_t Tag::get_id() const { return id_; }
    const std::string& Tag::get_name() const { return ecs::TagManager::instance().tag_name(id_); }
}