...
export void create_grass_blade_mesh()
{
    ...
    vertices.reserve((num_segments + 1) * 2 + 1); // Each segment has 2 verts, plus tip vertex
    indices.reserve(num_segments * 6 + 3);        // 2 triangles per segment, 1 for tip
    ...
    for (int seg = 0; seg <= num_segments; ++seg)
    {
        ...
        vertices.emplace_back(
            glm::vec3(-half_width, y, curve_offset),
            normal,
            glm::vec2(0.0f, v_coord)
        );

        vertices.emplace_back(
            glm::vec3(half_width, y, curve_offset),
            normal,
            glm::vec2(1.0f, v_coord)
        );
    }
    ...
    const std::uint32_t     tip_idx    = static_cast<std::uint32_t>(vertices.size() - 1);
    ...
    Mesh::create("grass_blade", std::move(vertices), std::move(indices));
}
